...
class ContactController extends Controller
{
    ...
    public function game_confirm(Request $request)
    {
        //バリデーションを実行（結果に問題があれば処理を中断してエラーを返す）
        $request->validate([
            'email' => 'required|email',
        ...
        //入力内容確認ページのviewに変数を渡して表示
        return view('game_confirm', [
            'inputs' => $inputs,
        ]);
    }
    public function game_send(Request $request)
    {
        //バリデーションを実行（結果に問題があれば処理を中断してエラーを返す）
        $request->validate([
            'email' => 'required|email',
        ...
        if($action !== 'submit'){
            ...
            //入力されたメールアドレスにメールを送信
            \Mail::to($inputs['email'])->send(new GameMail($inputs));
            \Mail::to(config('mail.username'))->send(new GameMail($inputs));
            //再送信を防ぐためにトークンを再発行
            $request->session()->regenerateToken();

            //送信完了ページのviewを表示
            return view('game_thanks');
